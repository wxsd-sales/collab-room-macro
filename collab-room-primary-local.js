...
const config = {
  commonUsername: 'admin',  // Common local username on the devices
  commonPassword: 'password123', // Common local password on the devices
  devices: [
    '192.168.1.2',         // Array of device IP addresses
    '172.16.1.2'
  ]
};


const ip_callout = new GMM.Connect.IP(config.commonUsername, config.commonPassword, config.devices) 
...
async function callSuccess(event) {
  ...
  console.log('Notifying other devices to dial:  ' + number.split(':')[1]);

  ip_callout.command({ dial: number }).post();
...
function callDisconnect(event) {
  ...
  ip_callout.command({ disconnect: event.RemoteURI }).post();
}

//ip_callout.command({ dial: 'wimills@alpha.webex.com' }).post();
